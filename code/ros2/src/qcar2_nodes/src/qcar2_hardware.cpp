...
public:
    ...
    : Node("qcar2")
    {
        ...
        speed_publisher_ = this->create_publisher<std_msgs::msg::Float32>("qcar2_speed", 10, pub_options);
        ...
        if (desired_speed != 0)
{
    ...
    if (speed_publisher_)
    {
        std_msgs::msg::Float32 spd_msg;
        spd_msg.data = static_cast<float>(measured_speed);
        speed_publisher_->publish(spd_msg);
    }

    speed_error = desired_speed - measured_speed;
    motor_speed_cmd = motor_speed_cmd
        + (speed_error*kp + ((speed_error - prior_speed_error)/delta_time.seconds())*kd)
        * 0.0047 / battery_voltage;
...
else
{
    ...
    // 정지 상태에서도 속도 0을 publish (PlotJuggler용)
    if (speed_publisher_)
    {
        std_msgs::msg::Float32 spd_msg;
        spd_msg.data = 0.0f;
