...
public:
  ...
  : Node("stop_command_node")
  {
    ...
    stop_publisher_ = this->create_publisher<std_msgs::msg::Int32>("yolo_stop", 10);
...
private:
  ...
  {
    stop_ = 0;  // 👈 클래스 멤버 변수로 선언한 stop_ 사용
    float x = msg->x;
    float y = msg->y;
    ...
    if (current_index_ < target_coords_.size())
    {
      ...
      if (distance < 0.1) 
      {
        stop_ = 1;  // 👈 클래스 멤버 변수로 선언한 stop_ 사용
    ...
    stop_msg.data = stop_;  // 👈 클래스 멤버 변수로 선언한 stop_ 사용
